...
std::string ParamsFilesGenerator::generateYarpdevFile()
{
    ...
    for (auto param : m_params)
    {
        if (param.getListOfGroups().size()==0)
        {
            s << param.getParamOnly() << " ";
        ...
        {
            //@@@TODO: This is not guaranteed to work if multiple parameters belong
            //to same section.
            std::deque<std::string> q = param.getListOfGroups();
            do
            {
                s << "\n";
                s << "[ " << q.front() << " ]\n";
