...
bool StateExtendedInputPort::getLastSingle(int j, int field, double *data, Stamp &stamp, double &localArrivalTime)
{
    ...
    if (ret)
    {
        switch(field)
        {
            ...
            default:
                yCError(CONTROLBOARD_NWC_YARP) << "RemoteControlBoard internal error while reading data. Cannot get 'single' data of type " << yarp::os::Vocab32::decode(field);
            break;
        }
...
bool StateExtendedInputPort::getLastVector(int field, double* data, Stamp& stamp, double& localArrivalTime)
{
    ...
    if (ret)
    {
        switch(field)
        {
            ...
            default:
                yCError(CONTROLBOARD_NWC_YARP) << "RemoteControlBoard internal error while reading data. Cannot get 'vector' data of type " << yarp::os::Vocab32::decode(field);
            break;
        }
