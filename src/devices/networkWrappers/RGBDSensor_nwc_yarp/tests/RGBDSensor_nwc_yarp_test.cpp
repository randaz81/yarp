...
TEST_CASE("dev::RGBDSensor_nwc_yarp", "[yarp::dev]")
{
    ...
    SECTION("Checking RGBDSensor_nwc_yarp device with fakeDepthCamera")
    {
        ...
        // ictl is valid because the nwc supports it
        // the following, instead, will fail, because the nws receives it,
        // but the serverImpl does not have a valid pointer to the interface
        // of the fakedevice which it does not implement it.
    ...
    SECTION("Checking RGBDSensor_nwc_yarp device with fakeDepthCamera_2")
    {
        PolyDriver dddepth;
        PolyDriver ddnws;
        PolyDriver ddnwc;
        ...
        {
            Property pdepth_cfg;
            pdepth_cfg.put("device", "fakeDepthCamera_2");
            pdepth_cfg.put("width", 32);  //smaller frame to improve valgrind speed
            pdepth_cfg.put("height", 24); //smaller frame to improve valgrind speed
            REQUIRE(dddepth.open(pdepth_cfg));
        }
