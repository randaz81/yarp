...
bool Rangefinder2DWrapper::open(yarp::os::Searchable &config)
{
    ...
    yCWarning(RANGEFINDER2DWRAPPER) << "The old device is no longer supported, and it will be deprecated in YARP 3.7 and removed in YARP 4.";
...
void Rangefinder2DWrapper::run()
{
    if (sens_p!=nullptr)
    {
        ...
        if (ret)
        {
            ...
            yarp::dev::LaserScan2DSerializer& b = streamingPort.prepare();
            //b.clear();
            b.get().scans=ranges;
            b.get().angle_min= minAngle;
            b.get().angle_max= maxAngle;
            b.get().range_min= minDistance;
            b.get().range_max= maxDistance;
            b.get().status=status;
